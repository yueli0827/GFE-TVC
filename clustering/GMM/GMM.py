...
def choose_k_deepdpm_style_gmm(
    X: np.ndarray,
    k_min: int,
    k_max: int,
    seed: int,
...
def run_gmm(X: np.ndarray, k_mode: str, k_min: int, k_max: int, seed: int) -> Tuple[np.ndarray, Dict[str, Any]]:
    X = X.astype(np.float32, copy=False)
    ...
    K, bic_map = choose_k_deepdpm_style_gmm(X, k_min=k_min, k_max=k_max, seed=seed)
    ...
    labels = gmm.fit_predict(X).astype(np.int64)
...
def main() -> None:
    ...
    log("Clustering")
    labels, info = run_gmm(X, k_mode=str(args.k_mode), k_min=int(args.k_min), k_max=int(args.k_max), seed=0)
